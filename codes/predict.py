...
def predict_stock(symbol, period, sim, future):
    ...
    df = pdr.get_data_yahoo(symbol, period - timedelta(days=period), interval="1d")
    df.to_csv('data.csv')
    ...
    for r in results:
        if (
            (np.array(r)[-test_size:] < np.min(df['Close'])).sum() == 0
            and (np.array(r)[-test_size:] > np.max(df['Close']) * 2).sum() == 0
        ):
    ...
    for no, r in enumerate(accepted_results):
        labels = [f"<table style='border: 1px solid black; font-weight:bold; font-size: larger; background-color:white'>\
    ...
    lines = plt.plot(df.iloc[:, 0].tolist()[::5], df['Close'][::5], label='true trend', color='black', marker="*")
    labels = [
        f"<table style='border: 1px solid black; font-weight:bold; font-size:larger; background-color:white'>\
         <tr style='border: 1px solid black;'>\
         <th style='border: 1px solid black;'>Date:</th>\
    ...
    plt.legend()
    plt.title(f"Stock: {symbol}, Average Accuracy: {np.mean(accuracies)}")
    plt.gca().set_facecolor('white')
    _ = range(len(results[0]))
    plt.xticks([])
